...
// Shared color palette (same as SpeechBubble / ThoughtBubble)
function getTypeColor(type: DialogType, isPlan: boolean): { fill: number; border: number } {
  if (isPlan) return { fill: 0x6b4c00, border: 0xff8800 };
  switch (type) {
...
export class DecisionBubble {
  ...
  constructor(
    ...
    const { fill, border } = getTypeColor(type, this.isPlan);
    let fillColor = fill;
  ...
  private createBubble(fillColor: number, borderColor: number): Phaser.GameObjects.Container {
    ...
    by = Phaser.Math.Clamp(by, 4, cam.height - height - 4);
