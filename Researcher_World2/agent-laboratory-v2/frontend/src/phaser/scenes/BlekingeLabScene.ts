...
export class BlekingeLabScene extends BaseScene {
  ...
  preload() {
    ...
    try {
      ...
      this.load.on('progress', (value: number) => {
        ...
        // Non chiamiamo updateDebugInfo qui per evitare errori di texture
      });
      
      this.load.on('complete', () => {
        console.log('All assets loaded successfully');
        this.assetsLoaded = true;
        // Aggiorniamo il debug info solo dopo il caricamento completo
      ...
      this.load.on('loaderror', (file: any) => {
        ...
        // Aggiorniamo il debug info solo se il debugText è già stato creato
        if (this.debugText && this.debugText.scene) {
      ...
      /* 
      // Percorsi alternativi commentati che abbiamo testato:
      
      // 1. Test come normale immagine
      this.load.image('professor_img', 'assets/sprites/professor.png');
      this.load.image('researcher_img', 'assets/sprites/researcher.png');
      this.load.image('student_img', 'assets/sprites/student.png');
      this.load.image('doctor_img', 'assets/sprites/doctor.png');
      
      // 2. Test in assets/sprites/
      this.load.spritesheet('professor_sprites', 'assets/sprites/professor.png', { 
        frameWidth: 32, 
        frameHeight: 48 
      });
      
      // 3. Test percorsi alternativi
      this.load.image('professor_chars', 'assets/characters/professor.png');
      this.load.image('researcher_chars', 'assets/characters/researcher.png');
      this.load.image('student_chars', 'assets/characters/student.png');
      this.load.image('doctor_chars', 'assets/characters/doctor.png');
      
      // 4. Test con Agent/ sottocartella
      this.load.image('professor_agent', 'assets/sprites/Agent/professor.png');
      this.load.image('researcher_agent', 'assets/sprites/Agent/researcher.png');
      */
  ...
  create() {
    ...
    try {
      ...
      // Test di caricamento delle immagini in diverse varianti
      // Commenta la riga seguente per disabilitare il test visivo delle varianti
      // this.createLoadingVariantTest();
      
      // Crea texture di placeholder se necessario
      this.createMissingTextures();
      
      // Visualizza sprite grezzi per test
      // Commenta la riga seguente per disabilitare il test visivo degli sprite grezzi
      // this.createRawSpriteTest();
      ...
      // Titolo del laboratorio con effetto 3D in stile pixel art su sfondo bianco
        // Primo creiamo un container per tutto il titolo
        ...
        glowEffect.fillStyle(0x4fc3f7, 0.1);
        ...
        // Aggiungiamo piccoli "pixel" decorativi agli angoli in stile pixel art
        const pixelCorners = this.add.graphics();
        pixelCorners.fillStyle(0x4fc3f7, 1);
        // Angolo in alto a sinistra
        pixelCorners.fillRect(-textWidth/2, -textHeight/2, 6, 6);
        // Angolo in alto a destra
        pixelCorners.fillRect(textWidth/2 - 6, -textHeight/2, 6, 6);
        // Angolo in basso a sinistra
        pixelCorners.fillRect(-textWidth/2, textHeight/2 - 6, 6, 6);
        // Angolo in basso a destra
        pixelCorners.fillRect(textWidth/2 - 6, textHeight/2 - 6, 6, 6);
        pixelCorners.setDepth(12);
        titleContainer.add(pixelCorners);
  ...
  /**
   * Crea una visualizzazione diretta delle sprite per debug
   */
  private createRawSpriteTest(): void {
    try {
      console.log('Creating raw sprite test');
      
      // Pulisci sprite precedenti
      this.rawSprites.forEach(sprite => sprite.destroy());
      this.rawSprites = [];
      ...
      // Crea sprite direttamente senza utilizzare container o helper
      texturesToTest.forEach((key, index) => {
        const spriteX = x + (index * spacing);
        
        if (this.textures.exists(key)) {
          console.log(`Creating test sprite for ${key}`);
          
          // Crea uno sprite base
          const sprite = this.add.sprite(spriteX, y, key, 0);
          sprite.setScale(2);
          sprite.setAlpha(1);
          ...
          // Aggiungi etichetta
          const label = this.add.text(spriteX, y + 50, key, {
            fontSize: '14px',
            color: '#FFFFFF',
            backgroundColor: '#000000',
          ...
          // Visualizzazione del frame attuale
          const frameText = this.add.text(spriteX, y - 50, 'Frame: 0', {
            fontSize: '12px',
            color: '#FFFFFF',
            backgroundColor: '#000000',
          ...
          frameText.setDepth(100);
          
          // Pulsanti per cambiare frame
          const prevBtn = this.add.text(spriteX - 25, y, '<', {
            fontSize: '20px', 
          ...
          prevBtn.on('pointerdown', () => {
            ...
            const prevFrame = (parseInt(currentFrame) - 1 + texture.frameTotal) % texture.frameTotal;
            sprite.setFrame(prevFrame);
            frameText.setText(`Frame: ${prevFrame}`);
          });
          
          const nextBtn = this.add.text(spriteX + 25, y, '>', {
            fontSize: '20px', 
            backgroundColor: '#333333',
            padding: { left: 6, right: 6, top: 2, bottom: 2 }
          });
          ...
          nextBtn.on('pointerdown', () => {
            const texture = this.textures.get(key);
            const currentFrame = sprite.frame.name;
            const nextFrame = (parseInt(currentFrame) + 1) % texture.frameTotal;
            sprite.setFrame(nextFrame);
            frameText.setText(`Frame: ${nextFrame}`);
          });
          
          // Animazione test
          const playBtn = this.add.text(spriteX, y + 75, 'Play', {
            fontSize: '12px',
          ...
          // Pulsante di stop
          const stopBtn = this.add.text(spriteX, y + 100, 'Stop', {
            fontSize: '12px',
            backgroundColor: '#550000',
            padding: { left: 5, right: 5, top: 2, bottom: 2 }
          });
          stopBtn.setOrigin(0.5);
          stopBtn.setInteractive();
          stopBtn.on('pointerdown', () => {
            sprite.stop();
          });
          
          // Salva lo sprite per future operazioni
          this.rawSprites.push(sprite);
          
          // Aggiungi anche gli elementi UI alla lista per pulizia
          this.rawSprites.push(label, frameText, prevBtn, nextBtn, playBtn, stopBtn);
        } else {
          console.warn(`Cannot create test sprite: texture ${key} does not exist`);
        }
      });
      
      // Pulsante globale per nascondere/mostrare
      ...
      toggleButton.on('pointerdown', () => {
        // Toggle visibility di tutti gli sprite
        const visible = !this.rawSprites[0].visible;
        this.rawSprites.forEach(sprite => {
          sprite.setVisible(visible);
        });
      });
      
      this.rawSprites.push(toggleButton);
      
    } catch (error) {
      console.error('Error in createRawSpriteTest:', error);
    }
  }
...
private createAgentsFromConfig(): void {
    try {
      ...
      // Ottieni configurazione per Blekinge
      const blekingeConfig = AGENT_CONFIG.blekinge;
      
      if (!blekingeConfig || !blekingeConfig.agents || blekingeConfig.agents.length === 0) {
        console.warn('No agent configuration found for Blekinge');
        return;
      }
      
      // Crea agenti dalla configurazione
      blekingeConfig.agents.forEach((agentConfig: {
        type: string;
        name: string;
  ...
  protected createInteractionZones(): void {
    try {
      ...
      const relaxZone = this.add.zone(
        this.cameras.main.width - 100, 
        this.cameras.main.height - 100, 
        gridSize * 5, 
        gridSize * 3
      );
      relaxZone.setName('relax_area');
      relaxZone.setInteractive();
      
      // Debug: visualizza la zona
      zoneGraphics.lineStyle(2, 0x00ff00, 0.5);
      zoneGraphics.strokeRect(
        relaxZone.x - relaxZone.width / 2, 
        relaxZone.y - relaxZone.height / 2, 
        relaxZone.width, 
        relaxZone.height
      );
      
      this.interactionZones.push(relaxZone);
    } catch (error) {
