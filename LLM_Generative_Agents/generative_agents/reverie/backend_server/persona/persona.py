...
class Persona: 
  def __init__(self, name, folder_mem_saved=False, llm_model_type=None):
    # PERSONA BASE STATE 
    ...
    # Set the LLM model type for this persona
    if llm_model_type:
      self.scratch.llm_model_type = llm_model_type
    elif not hasattr(self.scratch, 'llm_model_type'):
      # Assign model based on persona characteristics
      self.scratch.llm_model_type = self._determine_model_type()

  def _determine_model_type(self):
    """
    Determine the appropriate LLM model type based on persona characteristics
    """
    # Check persona's innate traits and occupation
    if hasattr(self.scratch, 'currently'):
      currently = self.scratch.currently.lower()
      if 'artist' in currently or 'creative' in currently or 'paint' in currently:
        return "creative"
      elif 'research' in currently or 'professor' in currently or 'study' in currently:
        return "analytical"
      elif 'cafe' in currently or 'social' in currently or 'manager' in currently:
        return "social"
    
    # Check persona's personality
    if hasattr(self.scratch, 'personality'):
      personality = self.scratch.personality.lower()
      if 'creative' in personality or 'artistic' in personality:
        return "creative"
      elif 'analytical' in personality or 'logical' in personality:
        return "analytical"
      elif 'friendly' in personality or 'outgoing' in personality:
        return "social"
