...
# Aggiungi il percorso corretto per importare utils.py
# Il file utils.py si trova due livelli sopra (../../)
current_dir = os.path.dirname(os.path.abspath(__file__))
backend_server_dir = os.path.join(current_dir, '..', '..')
...
try:
    ...
    print(f"Backend server dir: {backend_server_dir}")
    print(f"Python path: {sys.path}")
    # Definisci le variabili essenziali se l'import fallisce
    openai_api_key = "not-needed-for-ollama"
    key_owner = "ollama-user"
    debug = True
...
OLLAMA_MODEL = "qwen3.5:4b"  # Modello multimodale con thinking e tool calling
EMBEDDING_MODEL = "nomic-embed-text"  # Per embeddings
EMBEDDING_CACHE = {}  # Cache per embeddings
RESPONSE_CACHE = {}  # Cache per risposte LLM per evitare richieste duplicate
MAX_CACHE_SIZE = 500  # Limite cache per gestione memoria

def manage_cache():
  """Gestisce la dimensione della cache per evitare overconsumption memoria"""
  if len(RESPONSE_CACHE) > MAX_CACHE_SIZE:
    # Rimuovi il 20% più vecchio quando superi il limite
...
def temp_sleep(seconds=0.01):  # Ridotto da 0.1 a 0.01 per velocità
  time.sleep(seconds)

def clean_ollama_response(response):
  """
  Pulisce la risposta di Ollama rimuovendo tag indesiderati e output verboso
  """
  import re
  
  # Rimuovi tag <think> e </think>
  ...
  # Pulisci linee che non sono task validi
  lines = response.split('\n')
  cleaned_lines = []
  for line in lines:
    line = line.strip()
    # Mantieni solo linee che sembrano task (numero seguito da parentesi)
    if line and (re.match(r'^\d+\)', line) or not line.startswith(('$', '\\', '**', '#', '`', '---', 'Thinking'))):
...
def ollama_chat_request(prompt, model=OLLAMA_MODEL, temperature=0.05, max_tokens=120):
  """
  Funzione helper per fare richieste chat a Ollama - Ultra ottimizzata
  """
  try:
    ...
    response = ollama.chat(
      ...
      options={
        ...
        'top_k': 5,        # Limita scelte per velocità
        'top_p': 0.2,      # Riduce variabilità 
        'repeat_penalty': 1.0,
        'num_ctx': 512,    # Contesto molto ridotto per velocità
        'verbose': False,  # Disabilita output verboso di thinking
      }
    )
    
    result = clean_ollama_response(response['message']['content'])
    RESPONSE_CACHE[cache_key] = result
    manage_cache()  # Gestisci dimensione cache
    return result
  except Exception as e:
    print(f"Errore Ollama: {e}")
    return "OLLAMA ERROR"

def ollama_generate_request(prompt, model=OLLAMA_MODEL, temperature=0.05, max_tokens=120):
  """
  Funzione helper per fare richieste generate a Ollama - Ultra ottimizzata
  """
  try:
    # Cache per evitare richieste duplicate
    ...
    result = clean_ollama_response(response['response'])
    RESPONSE_CACHE[cache_key] = result
    manage_cache()  # Gestisci dimensione cache
    return result
  except Exception as e:
...
def GPT4_request(prompt): 
  ...
  try:
    ...
    return ollama_chat_request(prompt, model=OLLAMA_MODEL, temperature=0.05, max_tokens=200)
  except: 
    print ("Ollama ERROR")
    return "Ollama ERROR"
...
def GPT_request(prompt, gpt_parameter): 
  ...
  try:
    ...
    temperature = gpt_parameter.get("temperature", 0.3)  # Default a 0.3 per più precisione
