...
OLLAMA_MODEL = "mistral-nemo"  # Usa mistral-nemo che hai appena installato
...
def clean_ollama_response(response):
  """
  Pulisce la risposta di Ollama rimuovendo tag indesiderati
  """
  # Rimuovi tag <think> e </think>
  if '<think>' in response:
    # Estrai solo il contenuto dopo </think>
    parts = response.split('</think>')
    if len(parts) > 1:
      response = parts[-1].strip()
    else:
      # Se non c'è </think>, prendi tutto dopo <think>
      response = response.split('<think>')[-1].strip()
  
  # Rimuovi altri tag comuni che potrebbero causare problemi
...
def ollama_generate_request(prompt, model=OLLAMA_MODEL, temperature=0.7, max_tokens=500):
  ...
  try:
    ...
    # Pulisci la risposta
    return clean_ollama_response(response['response'])
  except Exception as e:
    print(f"Errore Ollama: {e}")
    return "OLLAMA ERROR"
