...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    for count, i in enumerate(_cr): 
      ...
      if len(k) > 1:
        ...
        duration_str = duration_str.rstrip(')')
        try:
          duration = int(duration_str)
          cr += [[task, duration]]
        except ValueError:
          print(f"Warning: Could not parse duration '{duration_str}' for task '{task}'")
          continue
    ...
    if len(curr_min_slot) > total_expected_min: 
      ...
      if len(curr_min_slot) > 0:
        ...
        # Se curr_min_slot è vuoto, crea task di default
        default_task = ["resting", 0]
        for i in range(total_expected_min):
          curr_min_slot += [default_task]
  ...
  def get_default_task_breakdown(duration):
    """Genera una breakdown di default quando il parsing fallisce"""
    default_tasks = [
      ["preparing", 15],
      ["working", 30], 
      ["organizing", 15],
    ...
    while remaining > 0:
      for task, dur in default_tasks:
        if remaining <= 0:
          break
        actual_dur = min(dur, remaining)
        breakdown.append([task, actual_dur])
  ...
  # Validazione robusta dell'output
  for item in output:
    if isinstance(item, list) and len(item) >= 2:
      try:
        i_task = str(item[0])
        i_duration = int(item[1]) if isinstance(item[1], (int, float)) else 15  # Default 15 min
        
        # Filtra task con contenuto verboso
        if any(phrase in i_task for phrase in ["Okay, let's", "TOODOOOOOO", "The user", "Wait,", "Hmm,"]):
          i_task = "working"  # Task di default
          
        time_sum += i_duration
        if time_sum <= duration: 
          fin_output += [[i_task, i_duration]]
      except (ValueError, TypeError, IndexError):
        # Se parsing fallisce, usa task di default
        fin_output += [["working", 15]]
        time_sum += 15
    else:
      # Elemento malformato, usa default
      fin_output += [["working", 15]]
      time_sum += 15
      break
      
  # Se fin_output è vuoto o malformato, usa default
  if not fin_output or len(fin_output) == 0:
    print("FALLBACK: Using default task breakdown")
    fin_output = get_default_task_breakdown(duration)
  ...
  if fin_output and len(fin_output) > 0:
    fin_output[-1][1] += (duration - ftime_sum)
  output = fin_output 
