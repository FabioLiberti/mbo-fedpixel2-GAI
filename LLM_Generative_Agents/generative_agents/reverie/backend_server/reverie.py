...
class ReverieServer: 
  def __init__(self, 
    ...
    if not os.path.exists(fs_storage):
      os.makedirs(fs_storage, exist_ok=True)
    
    # Create movement directory in source if it doesn't exist
    fork_movement_dir = f"{fork_folder}/movement"
    if not os.path.exists(fork_movement_dir):
      os.makedirs(fork_movement_dir, exist_ok=True)
      # Create a dummy movement file
      dummy_movement = {"step": 0, "persona": {}, "meta": {"curr_time": "2023-02-13 00:00:00"}}
      with open(f"{fork_movement_dir}/0.json", "w") as f:
        json.dump(dummy_movement, f, indent=2)
    
    copyanything(fork_folder, sim_folder)
    
    # Ensure movement directory exists in destination
    dest_movement_dir = f"{sim_folder}/movement"
    if not os.path.exists(dest_movement_dir):
      os.makedirs(dest_movement_dir, exist_ok=True)
